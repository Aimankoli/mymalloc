...
Basic Information and working of the Library:
...
Error Checking and Testing Plan:
...
the chunks in an alternating pattern and filled them with a new byte pattern. 

The First test was to make sure that even after deallocating a chunk and replacing its
values with new ones we could be sure that the other chunks would remain unchanged. 
For the second  test, we tried to see what would happen if the client asked for memory the size of the heap. The heap can only allocate a maximum size of MEMLENGTH- sizeof(header)
The third tests checks if it is possible to allocate memory and then try to deallocate it twice. This will return a double free error.
The fourth test sees if you can attempt to free an address that is not at the beginning of the payload, for example, free(ptr+1). This will return an invalid pointer error.
The last check sees if it is possible to free an address that it located well outside of the bounds of the heap.
...
If you want to use memgrind to see the performance of the Library, use the makefile:

$make
$./memgrind

The output should be something like:
Task 1 mean runtime: 6.480000
Task 2 mean runtime: 115.280000
Task 3 mean runtime: 26.780000
